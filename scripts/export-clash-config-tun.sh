...
# 从服务器端 V2Ray 配置导出 Clash TUN 模式配置文件
# 使用方法：在服务器上执行 bash scripts/export-clash-config-tun.sh

set -e

echo "🔄 正在从服务器配置生成 Clash TUN 模式配置文件..."
echo ""

# 配置变量
V2RAY_CONFIG="/etc/v2ray/config.json"
V2RAY_CONTAINER="v2ray"
OUTPUT_FILE="clash-config-tun-exported.yaml"
...
echo "✅ Clash TUN 模式配置文件已生成: $OUTPUT_FILE"
echo ""
echo "📋 配置内容预览："
echo "---"
head -30 "$OUTPUT_FILE"
...
echo "   3. 导入到 Clash 客户端并启用 TUN 模式"
echo ""
